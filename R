# plotting all chr data in one with special chrs
```r
...
for (current_pop in all_populations) {
  ...
  special_plot<-ggplot(special_chrom_and_exp_data,aes(x=MEAN_FST,y=mean.abs.logFC))+
    geom_point(color="blue",size=3)+
    theme_bw()+
    labs(title = paste("Mean_Fst_",current_pop))
  ...
  special_plot<-special_plot+
    geom_point(data = second_special_chrom_and_exp_data, mapping = 
                 aes(x=MEAN_FST,y=mean.abs.logFC,color="red",size=2))
  ...
  for (i in ch_numbers) {
    ...
    special_plot<-special_plot+
      geom_point(data = chrom_and_exp_data, mapping = 
                   aes(x=MEAN_FST,y=mean.abs.logFC,alpha=005))
  ...
  ggsave(paste("C:/Users/Tharindu/OneDrive - McMaster University/for lab and research/Tharindu on Mac/lab/expression_data/filtered_again_fst/out/",pop,"_Mean_Fst_VS_exp_all_in_one.pdf",sep = ''),plot = special_plot,width = 15,height = 30)
  #*************************************
